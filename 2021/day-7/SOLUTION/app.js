  ...
  const episodesHTMLString = episodes.map( episode => {
    ...
    return `<li>
          <label for="episode-${episode.id}">
            <input type="checkbox" ${checkedStr} name="episode-${episode.id}" id="episode-${episode.id}" onClick="handleCheckboxClick(event, ${episode.id})"/>
...
const handleCheckboxClick = (e, index) => {
  //if there is a first element and shift was down, check all the ones in between
  if(first && isShiftOn) {
    if(first < index) {
